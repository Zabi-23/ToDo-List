{
    ...
    function addTodo(text:string) {
        const newTodo: Todo ={       // skapa nya todo-uppifgt
            id: Date.now(), 
            text: text, 
            done: false,
        };
    ...
    function editTodo(id:number){
        ...
        const filteredTodos = todos.filter((t: Todo) => t.id === id);
    ...
    document.querySelector('#add-todo')?.addEventListener('click', () => addTodo(inputTodo.value));
    
    // **Lägg till eventlyssnare för att ta bort alla todo-uppgifter**
    document.querySelector('.todo-delete-all')?.addEventListener('click', clearTdos);
